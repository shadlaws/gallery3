 ...
 */
class Image_ImageMagick extends Image_ImageMagick_Driver {
  /**
   ...
   *
   * For ImageMagick, interlace *must* be done in the last "convert" execution or else it may not
  ...
  public function process($image, $actions, $dir, $file, $render = FALSE, $background = NULL)
  {
    ...
    if ($status = $this->execute($actions))
    {
      ...
      if ($error = exec(escapeshellcmd($this->dir.'convert'.$this->ext).' -background '.$this->background.' -flatten -quality '.$quality.'% '.$interlace.$strip.$this->cmd_image.' '.$this->new_image))
      // END: modifications to include interlace and strip
      {
        $this->errors[] = $error;
      }
